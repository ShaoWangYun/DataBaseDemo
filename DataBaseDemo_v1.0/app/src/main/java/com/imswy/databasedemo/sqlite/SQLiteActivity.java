...
import com.imswy.databasedemo.litepal.LitepalDBOperator;
...
public class SQLiteActivity extends AppCompatActivity{
        ...
        btnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!Name.isEmpty() && !StudentID.isEmpty() && !StudentID.isEmpty() && !etAge.getText().toString().isEmpty()) {
                    ...
                    student_current = sqLiteDBOperator.isExit(StudentID);
                    if (student_current != null) {
                        if (student_current.getName().equals(Name) && student_current.getGender().equals(Gender) && student_current.getAge() == Age) {
                            Utils.showToast(SQLiteActivity.this, "保存失败：学生信息重复", 1);
                        } else {
                            ...
                            builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    try {
                                        ...
                                        //数据库操作更新学生表
                                        sqLiteDBOperator.updateStudent(new Student(Name,Gender,student_current.getStudentID(),Age));
                                        //更新显示,更新特定的item（局部刷新）
                                        Log.i("Litepal","updatePosition is : "+updatePosition);
                                        studentAdapter.notifyItemChanged(updatePosition);
                                        refreshRecyclerView();
                                        //提示用户更新成功
                                        Utils.showToast(SQLiteActivity.this, "更新成功", 1);
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                        Utils.showToast(SQLiteActivity.this, "更新失败", 1);
                                    }
                                    dialog.dismiss();
                                }
                            });
                        ...
                        try {
                            //数据库操作，为学生表添加一条数据
                            sqLiteDBOperator.addStudent(new Student(Name,Gender,StudentID,Age));
                            //更新UI显示
                            int addPosition = sqLiteDBOperator.findAllStudentData().size()-1;
                            studentAdapter.notifyItemInserted(addPosition);
                            refreshRecyclerView();
                            //提示用户保存成功
                            Utils.showToast(SQLiteActivity.this, "保存成功", 1);
                        } catch (Exception e) {
                            e.printStackTrace();
                            Utils.showToast(SQLiteActivity.this, "保存失败", 1);
                        }
                    }
                } else {
                    Utils.showToast(SQLiteActivity.this, "保存失败：请输入完整的信息", 1);
                }
            }
        });
    ...
    private void refreshRecyclerView() {
        list_studentdata = sqLiteDBOperator.findAllStudentData();
        studentAdapter = new StudentAdapter(list_studentdata);
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        try {
                            ...
                            //删除学生操作
                            sqLiteDBOperator.deleteStudent(studentAdapter.getItem(position).getStudentID());
                            //刷新UI
                            studentAdapter.notifyItemRemoved(deleteposition);
                            refreshRecyclerView();
                            //提示用户
                            Utils.showToast(SQLiteActivity.this, "删除成功", 1);
                        } catch (Exception e) {
                            e.printStackTrace();
                            Utils.showToast(SQLiteActivity.this, "删除失败", 1);
                        }
                        dialog.dismiss();
                    }
                });
